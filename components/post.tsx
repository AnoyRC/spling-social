...
const Posts: NextPage<Props> = (props: Props) => {
  ...
  return (
    <div className="bg-[#FFFFFF] border-[#166f00] border-b-[1px] w-[100%] h-fit dark:bg-[#10332E] dark:border-[#40675F]">
      ...
      <div>
        {props.post?.title && <h1 className="text-[#000000] text-2xl my-3 mx-7 font-[QuicksandLight] font-bold hover:cursor-pointer dark:text-gray-300" onClick={()=>{window.location.href = `/post/${props.post?.postId}`}}>{props.post?.title.length < 63 ? props.post?.title : props.post?.title.substring(0,63) + "..."}</h1>}
        <div className="flex h-fit width-[100%] mb-4">
          {props.post && <h1 className="text-[#000000] text-base mx-7 w-[60%] font-[Quicksand] dark:text-gray-300">{props.post?.text.length < 240 ? props.post?.text : props.post?.text.substring(0,239) + "..."}</h1>}
      ...
      <div className="flex mb-5">
        <div className="flex justify-start w-[70%] ml-5">
          {tags[0] && <div className="w-[fit] h-[fit] border-[#166f00] border-[1px] px-1 rounded-md ml-2 dark:border-[#40675F]">
            <h1 className="text-[#000000] font-[Quicksand] dark:text-gray-300">{tags[0]}</h1>
          </div>}
          {tags[1] && <div className="w-[fit] h-[fit] border-[#166f00] border-[1px] px-1 rounded-md ml-2 dark:border-[#40675F]">
            <h1 className="text-[#000000] font-[Quicksand] dark:text-gray-300">{tags[1]}</h1>
          </div>}
          {tags.length > 2 && (
            <div className="w-[fit] h-[fit] border-[#166f00] border-[1px] px-1 rounded-md ml-2 dark:border-[#40675F]">
              <h1 className="text-[#000000] font-[Quicksand] dark:text-gray-300">{`+${tags.length-2}`}</h1>
            </div>
          )}
        </div>
